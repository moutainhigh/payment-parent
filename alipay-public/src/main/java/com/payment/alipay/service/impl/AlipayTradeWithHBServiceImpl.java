...
import com.payment.alipay.model.builder.AlipayTradePayRequestBuilder;
import com.payment.alipay.model.builder.AlipayTradeQueryRequestBuilder;
import com.payment.alipay.model.result.AlipayF2FPayResult;
import com.payment.alipay.service.impl.hb.HbListener;
import com.payment.alipay.service.impl.hb.TradeListener;
    ...
    public static class ClientBuilder {
        ...
        public AlipayTradeWithHBServiceImpl build() {
            if (StringUtils.isEmpty(gatewayUrl)) {
                gatewayUrl = AlipayPropertiesConfig.getOpenApiDomain(); // 与mcloudmonitor网关地址不同
            }
            if (StringUtils.isEmpty(appid)) {
                appid = AlipayPropertiesConfig.getAppid();
            }
            if (StringUtils.isEmpty(privateKey)) {
                privateKey = AlipayPropertiesConfig.getPrivateKey();
            }
            ...
            if (StringUtils.isEmpty(alipayPublicKey)) {
                alipayPublicKey = AlipayPropertiesConfig.getAlipayPublicKey();
            }
            if (StringUtils.isEmpty(signType)) {
                signType = AlipayPropertiesConfig.getSignType();
            }
            if (listener == null) {
